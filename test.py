...
def run_each_dataset(model_dir, retinanet, dataset_path, subset, cur_dataset):	
	print("dataset_path  " , dataset_path, subset ,cur_dataset)
	mov ="IMG_4044"
	img_list = os.listdir(os.path.join(dataset_path, subset, cur_dataset, mov))
	img_list = [os.path.join(dataset_path, subset, cur_dataset, mov, _) for _ in img_list if ('jpg' in _) or ('png' in _)]
	...
	for idx in range(img_len + 1):
		i = idx - 1
		print('tracking : {} / {}'.format(i,img_len))
		with torch.no_grad():

			data_path1 = img_list[min(idx, img_len - 1)]
			#print(img_list[min(idx, img_len - 1)])
	...
	fout_tracking = open(os.path.join(model_dir, 'results', mov + '.txt'), 'w')

	save_img_dir = os.path.join(model_dir, 'results', cur_dataset)
	if not os.path.exists(save_img_dir):
		os.makedirs(save_img_dir)

	out_video = os.path.join(model_dir, 'results', mov + '.mp4')
	...
	for i in range(img_len):
		...
		cv2.waitKey(0)
	print(trace_len_draw , max_draw_len, trace_len)
	fout_tracking.close()
	videoWriter.release()
...
def main(args=None):
	...
	retinanet = torch.load(os.path.join(parser.model_dir, 'model_final_152.pt'))
	...
	run_each_dataset(parser.model_dir, retinanet, parser.dataset_path, 'test','test')
