...
class FocalLoss(nn.Module):
    def forward(self, classifications, regressions, anchors, annotations1, annotations2):
        ...
        regression_losses = []
        # classifications.shape  : ( 2 , 5456 , 1)
        #print(" classifications "  , classifications.shape , classifications)
        ...
        for j in range(batch_size):
            ...
            classification = classifications[j, :, :]	# (5456, 1)
            ...
            #print(,annotations1[j, :, :].shape) # , annotations2[j, :, :])
            #print("------------")
            #print(bbox_annotation ,bbox_annotation.shape ) #, bbox_annotation_next)
            ...
            #print("targets_0" , targets , targets.shape)
            targets[torch.lt(best_truth_overlap, 0.4), :] = 0
            #print(torch.lt(best_truth_overlap, 0.4)) 
            #print("targets_1" , targets , targets.shape)
            ...
            #print("targets[targets==1] " , targets[targets==1])
            #print("targets[targets==-1]" , targets[targets== -1])
            ...
            # ne : not euqal to =!
            #print(cls_loss)
            #print("cls_loss[cls_loss == 0].shape" ,cls_loss[cls_loss == 0] , cls_loss[cls_loss == 0].shape)
            cls_loss = torch.where(torch.ne(targets, -1.0), cls_loss, torch.zeros(cls_loss.shape).cuda())
            #print("cls_loss" , cls_loss.shape) #[ 5456 , 1]
            #print(cls_loss)
            #print("cls_loss[cls_loss == 0]")
            #print(cls_loss[cls_loss == 0]  , cls_loss[cls_loss == 0].shape)
            #print("targets[targets == -1]")
            #print(targets[targets == -1]   , targets[targets == -1].shape)
            #print(cls_loss[cls_loss == 0].shape == targets[targets == -1].shape) # True
