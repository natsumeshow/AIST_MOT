...
def main(args=None):
	...
	for epoch_num in range(parser.epochs):
		...
		for iter_num, data in enumerate(dataloader_train):
			try:
				...
				#torch.save(retinanet, os.path.join(parser.model_dir, 'model_final_{}.pt'.format(parser.depth)))
	...
	retinanet.eval()
	np.save("last.npy",loss_hist)
	np.save("last_all.npy",loss_hist)
	np.save("mean.npy" ,np.mean(loss_hist) )
	torch.save(retinanet, os.path.join(parser.model_dir, 'model_final_{}.pt'.format(parser.depth)))
